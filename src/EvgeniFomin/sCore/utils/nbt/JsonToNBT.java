...
public class JsonToNBT {
		...
		if (!this.canRead()) {
			...
			while (this.peek() != ']') {
				...
				if (i < 0) {
					...
					throw this.exception("Unable to insert " + NBTBase.getTagTypeName(j) + " into ListTag of type "
	...
	private <T extends Number> List<T> readArray(byte p_193603_1_, byte p_193603_2_) throws NBTException {
		...
		while (true) {
			if (this.peek() != ']') {
				...
				if (i != p_193603_2_) {
					throw this.exception("Unable to insert " + NBTBase.getTagTypeName(i) + " into "
	...
	private void expect(char expected) throws NBTException {
		...
		if (flag && this.peek() == expected) {
			...
			throw new NBTException("Expected '" + expected + "' but got '" + (flag ? this.peek() : "<EOF>") + "'",
					this.string, this.cursor + 1);
		}
	}

	protected boolean isAllowedInKey(char charIn) {
		return charIn >= '0' && charIn <= '9' || charIn >= 'A' && charIn <= 'Z' || charIn >= 'a' && charIn <= 'z'
				|| charIn == '_' || charIn == '-' || charIn == '.' || charIn == '+';
	}
